...
class PairDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #print('*******\n%s\n%s\n*******\n' % (A_path,B_path))
        #print(A_img)
        #A_img.save('/data/hzh/experiment/RetinexGAN-edge/checkpoints/1.png')
        #exit()
        ...
        if self.opt.resize_or_crop == 'no':
            ...
            A_gray = torch.unsqueeze(A_gray, 0)
            #print(A_gray.shape)
        return {'A': A_img, 'B': B_img, 'A_gray': A_gray, 'input_img':input_img, 'A_I':A_gray,
                'A_paths': A_path, 'B_paths': B_path}
        #return {'A': A_img, 'B': B_img, 'A_gray': A_gray, 'A_I':input_img,
                #'A_paths': A_path, 'B_paths': B_path}
